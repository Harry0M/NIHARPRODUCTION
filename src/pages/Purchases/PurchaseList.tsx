...
const PurchaseList = () => {
  ...
  const { data: purchasesData, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      console.log(`🔍 Purchase search: "${debouncedSearchTerm}", page: ${page}, pageSize: ${pageSize}`);
      ...
      if (debouncedSearchTerm.trim()) {
        ...
        console.log(`📊 Found ${count} total purchases matching criteria`);
        setTotalCount(count);
      } else {
        console.log(`📋 Loading all purchases (no search filter)`);
        ...
        console.log(`📊 Found ${count || 0} total purchases matching criteria`);
      ...
      if (debouncedSearchTerm.trim()) {
        ...
        console.log(`🔍 Applying data search filter: "${searchTerm}"`);
        ...
        console.log(`✅ Purchase search completed: found ${paginatedData.length} results`);
      ...
      console.log(`✅ Purchase search completed: found ${data?.length || 0} results`);
      return data as Purchase[];
      } catch (searchError) {
        console.error("Error in purchase search:", searchError);
        throw searchError;
      }
    }
  });
          ...
          <div className="my-16 flex flex-col items-center justify-center text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground max-w-sm">
              {debouncedSearchTerm.trim()
                ? `No purchases matching "${debouncedSearchTerm}" were found. Try a different search term.`
