...
const VendorBillsList = () => {
  ...
  const fetchAvailableJobs = async () => {
    try {
      ...
      const [cuttingJobs, printingJobs, stitchingJobs] = await Promise.all([        // Cutting jobs: completed + has vendor_id
        supabase
          .from('cutting_jobs')
          .select(`
            id,
  ...
  const handleCreateBill = (job: JobWithDetails) => {
    // Simple approach: navigate with job data in the URL params
    const searchParams = new URLSearchParams({
      jobType: job.job_type,
      jobId: job.id,
      vendorId: job.vendor_id || '',
      workerName: job.worker_name || '',
      orderNumber: job.job_cards?.orders?.order_number || '',
      companyName: job.job_cards?.orders?.company_name || '',
      quantity: job.received_quantity?.toString() || '0',
      rate: job.rate?.toString() || '0'
    });
    
    navigate(`/sells/vendor-bills/create?${searchParams.toString()}`);
  };
