...
const VendorBillCreateForm = () => {
  ...
  const fetchVendors = useCallback(async () => {
    try {
      const { data, error } = await supabase
        .from('vendors')
        .select('id, name, contact_person, phone, email, service_type')
        .eq('status', 'active')
        .order('name');
  ...
  const fetchJobDetails = useCallback(async () => {
    ...
    try {
      ...
      setJob({ ...data, job_type: jobType as 'cutting' | 'printing' | 'stitching' });
    } catch (error) {
      toast({
