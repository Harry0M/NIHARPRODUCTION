-- DEFINITIVE FIX: Create delete_order_completely function with correct parameter name
-- This ensures the function matches what the frontend expects

-- Drop any existing versions
DROP FUNCTION IF EXISTS public.delete_order_completely(uuid);
DROP FUNCTION IF EXISTS public.delete_order_completely(target_order_id uuid);
DROP FUNCTION IF EXISTS public.delete_order_completely(order_id uuid);

-- Create the function with the parameter name that matches the frontend call
...
BEGIN
  ...
  DELETE FROM public.invoice_items WHERE order_id = order_id;
  GET DIAGNOSTICS affected_rows = ROW_COUNT;
...
'Completely deletes an order and all related records. Handles proper cascade deletion sequence: sales → dispatch → job cards → production jobs → components → transactions → order. Material consumption reversal should be handled by frontend before calling this function.';
...
BEGIN
  IF EXISTS (
    ...
    RAISE NOTICE 'SUCCESS: delete_order_completely function created with correct parameter name (order_id)';
  ELSE
