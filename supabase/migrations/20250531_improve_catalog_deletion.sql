-- Improved catalog product deletion function
-- This ensures all related records are properly deleted in the correct order
...
BEGIN
  ...
  -- Check for any orders using this catalog item
  IF EXISTS (
    SELECT 1 FROM public.order_items WHERE catalog_id = input_catalog_id
  ) THEN
    RAISE EXCEPTION 'Cannot delete catalog product: It is being used in orders';
  END IF;
