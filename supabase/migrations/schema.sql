...
CREATE OR REPLACE FUNCTION "public"."calculate_component_consumption"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
BEGIN
  -- Calculate consumption based on dimensions and roll width if provided
  IF NEW.length IS NOT NULL AND NEW.width IS NOT NULL AND NEW.roll_width IS NOT NULL THEN
    NEW.consumption := ((NEW.length * NEW.width) / (NEW.roll_width * 39.39));
  END IF;
...
ALTER FUNCTION "public"."calculate_component_consumption"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."delete_order_completely"("order_id" "uuid") RETURNS boolean
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $_$
DECLARE
  job_card_ids uuid[];
  cutting_job_ids uuid[];
BEGIN
  -- Delete order dispatches (which will cascade to delete dispatch_batches due to ON DELETE CASCADE)
  DELETE FROM public.order_dispatches WHERE order_dispatches.order_id = $1;
  
  -- Get all job card IDs
  SELECT array_agg(id) INTO job_card_ids FROM public.job_cards WHERE job_cards.order_id = $1;
  
  -- If job cards exist, delete their related records first
  IF job_card_ids IS NOT NULL AND array_length(job_card_ids, 1) > 0 THEN
    -- Get all cutting job IDs
    SELECT array_agg(id) INTO cutting_job_ids FROM public.cutting_jobs 
      WHERE cutting_jobs.job_card_id = ANY(job_card_ids);
    
    -- Delete cutting components
    IF cutting_job_ids IS NOT NULL AND array_length(cutting_job_ids, 1) > 0 THEN
      DELETE FROM public.cutting_components WHERE cutting_components.cutting_job_id = ANY(cutting_job_ids);
    END IF;
    
    -- Delete all related production jobs
    DELETE FROM public.cutting_jobs WHERE cutting_jobs.job_card_id = ANY(job_card_ids);
    DELETE FROM public.printing_jobs WHERE printing_jobs.job_card_id = ANY(job_card_ids);
    DELETE FROM public.stitching_jobs WHERE stitching_jobs.job_card_id = ANY(job_card_ids);
    
    -- Delete job cards
    DELETE FROM public.job_cards WHERE job_cards.order_id = $1;
  END IF;
  
  -- Delete order components
  DELETE FROM public.order_components WHERE order_components.order_id = $1;
  
  -- Finally delete the order itself
  DELETE FROM public.orders WHERE orders.id = $1;
...
ALTER FUNCTION "public"."delete_order_completely"("order_id" "uuid") OWNER TO "postgres";
...
BEGIN
  ...
  year_prefix := 'ORD-' || to_char(current_date, 'YYYY') || '-';
  
  -- Generate a unique number using a dedicated sequence
  -- This is more reliable than counting existing rows
  sequence_number := LPAD(nextval('order_number_seq')::TEXT, 4, '0');
  new_order_number := year_prefix || sequence_number;
  
  -- Set the order number
  NEW.order_number := new_order_number;
  
  -- Added for logging/debugging purposes
  RAISE NOTICE 'Generated order number: %', NEW.order_number;
...
CREATE OR REPLACE FUNCTION "public"."sync_catalog_changes"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  -- When catalog is updated, update product_details
  IF (TG_OP = 'UPDATE') THEN
    ...
    SET 
      ...
      height = NEW.height,
      default_quantity = NEW.default_quantity,
      default_rate = NEW.default_rate,
      selling_rate = NEW.selling_rate,
  ...
  -- When a new catalog entry is created, create a corresponding product_details
  IF (TG_OP = 'INSERT') THEN
    INSERT INTO product_details (
      ...
      NEW.bag_length,
      NEW.bag_width,
      NEW.border_dimension,
      NEW.height,
      NEW.default_quantity,
      NEW.default_rate,
      NEW.selling_rate,
      NEW.total_cost,
      NEW.margin,
      NEW.cutting_charge,
      NEW.printing_charge,
      NEW.stitching_charge,
      NEW.transport_charge
    );
  END IF;
  
  -- When a catalog entry is deleted, the product_details will be deleted by CASCADE
...
BEGIN
  ...
  -- Calculate base consumption if we have length, width and roll_width
...
ALTER FUNCTION "public"."update_catalog_component_consumption"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."update_inventory_from_purchase"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
  item_record RECORD;
...
BEGIN
  -- Only update inventory when status changes to 'completed'
  IF (TG_OP = 'UPDATE' AND NEW.status = 'completed' AND OLD.status != 'completed') THEN
    ...
    LOOP
      ...
      SELECT * INTO material_record FROM public.inventory WHERE id = item_record.material_id;
      
      -- Update inventory quantity
      UPDATE public.inventory
      SET 
        quantity = quantity + item_record.quantity,
        purchase_rate = item_record.unit_price,
        updated_at = now()
      ...
      INSERT INTO public.inventory_transaction_log (
        ...
        material_record.quantity + item_record.quantity,
        NEW.id::text,
        NEW.purchase_number,
        'Purchase',
      ...
      -- Create inventory transaction for compatibility
      INSERT INTO public.inventory_transactions (
        material_id,
        inventory_id,
        transaction_type,
...
ALTER FUNCTION "public"."update_inventory_from_purchase"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."update_material_info"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
...
BEGIN
  -- Get material details
  SELECT * INTO v_material_record FROM public.inventory WHERE id = NEW.material_id;
  
  -- Update component with material details if available
  IF FOUND THEN
    -- Only update if values are not already set
    IF NEW.gsm IS NULL AND v_material_record.gsm IS NOT NULL THEN
      NEW.gsm := v_material_record.gsm::numeric;
    END IF;
    
    IF NEW.color IS NULL AND v_material_record.color IS NOT NULL THEN
      NEW.color := v_material_record.color;
    END IF;
  END IF;
...
ALTER FUNCTION "public"."update_material_info"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."orders" (
    ...
    "stitching_charge" numeric DEFAULT 0,
    "transport_charge" numeric DEFAULT 0,
    CONSTRAINT "company_name_or_id" CHECK (((("company_name" IS NOT NULL) AND ("company_id" IS NULL)) OR (("company_name" IS NULL) AND ("company_id" IS NOT NULL))))
);
...
CREATE OR REPLACE TRIGGER "calculate_catalog_consumption" BEFORE INSERT OR UPDATE ON "public"."catalog_components" FOR EACH ROW EXECUTE FUNCTION "public"."update_catalog_component_consumption"();



CREATE OR REPLACE TRIGGER "catalog_component_consumption_trigger" BEFORE INSERT OR UPDATE OF "length", "width", "roll_width", "size" ON "public"."catalog_components" FOR EACH ROW EXECUTE FUNCTION "public"."update_catalog_component_consumption"();
...
CREATE OR REPLACE TRIGGER "tr_calculate_component_consumption" BEFORE INSERT OR UPDATE ON "public"."catalog_components" FOR EACH ROW EXECUTE FUNCTION "public"."calculate_component_consumption"();
...
CREATE OR REPLACE TRIGGER "trigger_sync_catalog_changes" AFTER INSERT OR UPDATE ON "public"."catalog" FOR EACH ROW EXECUTE FUNCTION "public"."sync_catalog_changes"();



CREATE OR REPLACE TRIGGER "trigger_update_inventory_from_purchase" AFTER UPDATE ON "public"."purchases" FOR EACH ROW EXECUTE FUNCTION "public"."update_inventory_from_purchase"();
...
CREATE OR REPLACE TRIGGER "update_material_info_trigger" BEFORE INSERT OR UPDATE ON "public"."catalog_components" FOR EACH ROW WHEN (("new"."material_id" IS NOT NULL)) EXECUTE FUNCTION "public"."update_material_info"();
...
GRANT ALL ON FUNCTION "public"."calculate_component_consumption"() TO "anon";
GRANT ALL ON FUNCTION "public"."calculate_component_consumption"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."calculate_component_consumption"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."delete_order_completely"("order_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."delete_order_completely"("order_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."delete_order_completely"("order_id" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_catalog_component_consumption"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_catalog_component_consumption"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_catalog_component_consumption"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_inventory_from_purchase"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_inventory_from_purchase"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_inventory_from_purchase"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_material_info"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_material_info"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_material_info"() TO "service_role";
